...
impl RootInner for WindowsWindow {
    fn application(&self) -> &dyn controls::Application {
        unsafe { common::cast_hwnd::<application::Application>(self.app) }
    }
...
impl WindowInner for WindowsWindow {
    fn with_params(app: common::Hwnd, title: &str, window_size: types::WindowStartSize, _menu: types::Menu) -> Box<Window> {
        unsafe {
            ...
            let hwnd = winuser::CreateWindowExW(
                ...
                ptr::null_mut(),//app.as_inner().root.into(),
                ptr::null_mut(),
                hinstance(),
