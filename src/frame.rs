...
impl FrameInner for WindowsFrame {
	fn with_label(label: &str) -> Box<Frame> {
		let b = Box::new(Member::with_inner(Control::with_inner(SingleContainer::with_inner(
			WindowsFrame {
        ...
        if self.child.is_some() {
        	if !self.base.hwnd.is_null() {
	        	...
	        	if let Some(new) = self.child.as_mut() {
		        	new.as_mut().on_added_to_container(common::member_from_hwnd::<Frame>(self.base.hwnd), w as i32 - DEFAULT_PADDING, h as i32 - DEFAULT_PADDING);
		        }
		    }
        }
        self.base.invalidate();
...
impl ControlInner for WindowsFrame {
	fn on_added_to_container(&mut self, member: &mut MemberBase, control: &mut ControlBase, parent: &controls::Container, px: i32, py: i32) {
        ...
        let (hwnd, hwnd_gbox, id) = unsafe {
            ...
            let (hwnd, id) = common::create_control_hwnd(px,
        ...
        if let Some((x, y)) = self.base.coords {
            unsafe {
                winuser::SetWindowPos(self.base.hwnd,
                                      ...
                                      self.base.measured_size.1 as i32 - self.label_padding,
                                      0);
